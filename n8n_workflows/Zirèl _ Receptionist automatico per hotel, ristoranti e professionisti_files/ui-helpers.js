...
(function () {
    ...
    function hydrateFaqIcons() {
        document.querySelectorAll('.faq-icon').forEach(icon => {
            if (icon.querySelector('.faq-chevron')) return;

            icon.innerHTML = '<svg class="faq-chevron" viewBox="0 0 16 16" aria-hidden="true"><path d="M3.5 6.25 8 10.75l4.5-4.5"></path></svg>';
        });
    }

    function normalizeFaqMarkup() {
        document.querySelectorAll('.faq-answer').forEach(answer => {
            const firstChild = answer.firstElementChild;
            if (firstChild && firstChild.tagName === 'P' && !firstChild.classList.contains('faq-answer-content')) {
                firstChild.classList.add('faq-answer-content');
            }
        });
    }
    ...
    function inferDestinationType(href) {
        if (!href) return 'unknown';
        if (href.startsWith('https://wa.me/')) return 'whatsapp';
        if (href.startsWith('tel:')) return 'phone';
        if (href.startsWith('mailto:')) return 'email';
        if (href.includes('pricing')) return 'pricing';
        if (href.includes('demo') || href.startsWith('#')) return 'demo';
        if (href.includes('contatti')) return 'contact';
        if (href.includes('guide')) return 'guide';
        if (href.includes('login') || href.includes('register')) return 'auth';
        return 'page';
    }

    function trackCtaClick(target) {
        if (!target || typeof window.va !== 'function') return;

        const href = target.getAttribute('href') || '';
        const pageType = document.body.dataset.pageType || 'site';
        const location = target.dataset.trackLocation || pageType;
        const label = target.dataset.trackLabel || target.textContent.trim();
        const sector = target.dataset.trackSector || inferSector(pageType) || inferSector(location);
        const destinationType = inferDestinationType(href);
        ...
        const payload = {
            ...
            channel: destinationType === 'whatsapp' ? 'whatsapp' : destinationType === 'phone' ? 'phone' : 'web',
            is_external: href.startsWith('http') ? 'true' : 'false',
            page_path: window.location.pathname
        };

        try {
            window.va('event', 'cta_click', payload);
        } catch (error) {
    ...
    document.addEventListener('click', event => {
        const trackable = event.target.closest('[data-track="cta"]');
        if (!trackable) return;
        trackCtaClick(trackable);
    });
