        ...
        <div class="bg-white w-[350px] h-[500px] rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-brand-sand">
            <div class="brand-gradient p-5 text-white flex justify-between items-center">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h6 class="font-bold text-sm">L'Oste di Zirèl</h6>
                        <p class="text-[10px] opacity-70">Accoglienza romagnola h24</p>
                    </div>
                </div>
            ...
            <div class="flex-grow p-4 space-y-4 bg-brand-sand/10 overflow-y-auto" id="chat-messages">
                <div class="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm max-w-[90%] text-sm border border-brand-sand/50">
                    <p>Ben arrivato! 👋 Sono l'Oste di <strong class="text-brand-orange font-sans">Zirèl</strong>.</p>
                    <p>Come posso aiutarti oggi?</p>
                </div>

                <div class="quick-replies" id="quick-replies-container">
                    <button class="quick-reply-btn"
                        onclick="sendChatMessage('🍽️ Mostrami il Menù')">
                        🍽️ Mostrami il Menù
                    </button>
                    <button class="quick-reply-btn" onclick="sendChatMessage('🕒 Quali sono gli orari?')">
                        🕒 Quali sono gli orari?
                    </button>
                    <button class="quick-reply-btn"
                        onclick="sendChatMessage('📍 Dove siete?')">
                        📍 Dove siete?
                    </button>
                    <button class="quick-reply-btn" onclick="sendChatMessage('📞 Voglio parlare con lo staff')">
                        📞 Parla con lo staff
                    </button>
                </div>
            </div>
            ...
            <div class="p-4 border-t border-brand-sand flex gap-2 items-center bg-white">
                <input type="text" id="chat-input-field" placeholder="Scrivi all'Oste..."
            ...
            try {
                ...
                const response = await fetch(webhookUrl, {
                    ...
                    body: JSON.stringify({
                        ...
                        sessionId: sessionId,
                        metadata: {
                            tenant_id: 'chiringuito_gino_001'
                        }
                    })
                });
                ...
                let formattedText = botText.replace(/\\*\\*(.*?)\\*\\*/g, '<strong class="font-bold text-brand-orange">$1</strong>');
...
const replaceRegex = /<!-- Chat configuration and instantiation -->[\s\S]*?(?=<footer)/;
if (content.match(replaceRegex)) {
    content = content.replace(replaceRegex, chatHtml + scriptBody + '\n    <footer');
    fs.writeFileSync(path, content);
    console.log("Successfully patched demo.html");
} else {
    console.log("Could not find the target block in demo.html");
}
